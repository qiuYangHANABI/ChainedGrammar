...
- (void)request:(NSString *)method URL:(NSString *)urlString parameters:(id)parameters completion:(void (^)(LMJBaseResponse *response))completion
{
    ...
    //定义success回调，请求成功后将获取的responseObjec、task进行下一步处理
    ...
    if ([method isEqualToString:@"GET"]) {
        //调用AFNetworking接口请求数据，并通过传入的success和failure获得回调信息
...
- (void)configSettings
{
    //设置可接收的数据类型(AFHTTPResponseSerializer对象用于处理)
    ...
    //自定义处理数据(可以在此处对接收的response进行自定义处理然后通过返回值传出)
...
- (void)setResponseSerializer:(AFHTTPResponseSerializer<AFURLResponseSerialization> *)responseSerializer
{
    ...
    if ([responseSerializer isKindOfClass:[AFJSONResponseSerializer class]]) {
        ...
        //当返回的数据为null，则替换为nil，解决了向NSNull类型数据发送消息崩溃的问题
